...
from .base import MissionWriter


_HEADER = "QGC WPL 110"


class QGCWPLWriter(MissionWriter):
    """Writer that outputs QGC WPL 110 format."""

    def format(self, items: list[MissionItem]) -> str:
        """Format mission items as a QGC WPL 110 string."""
        lines = [_HEADER]
        for item in items:
...
def to_string(items: list[MissionItem]) -> str:
    ...
    return QGCWPLWriter().format(items)
...
def write_waypoints(items: list[MissionItem], path: str | Path) -> Path:
    ...
    """
    return QGCWPLWriter().write(items, path)
